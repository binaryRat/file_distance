...
static char* get_changed_file(char*,Data*,char*);
...
static char* get_changed_file(char* origin_buffer,Data* changes,char* result_buffer){

	if(result_buffer == NULL) return NULL;

	int origin_size = strlen(origin_buffer);
...
int change_file(char *origin_file, char *file_mod,char *output_file) {
	...
	char *file_1 = read_file(origin_file);

	if(file_1 != NULL){

		Data *file_2 = (Data*)read_file(file_mod);

		if(file_2 != NULL){

			long number_of_mods = get_file_size(file_mod) / sizeof(Data);
	
			int fsize = predict_file_size(strlen(file_1), number_of_mods,file_2);
